...
def _update_slider():
    ...
    level = st.selectbox('Select Parliament:', df['P'].dropna().unique().tolist(), index=df['P'].dropna().unique().index(selected_row["Parliament"].values[0]))
    filtered_df = filter_data(level)
    d_name = st.selectbox('Select District:', filtered_df['D'].dropna().unique().tolist(), index=filtered_df['D'].dropna().unique().index(selected_row["District"].values[0]))
    for i, column_name in enumerate(renamed_columns.values()):
